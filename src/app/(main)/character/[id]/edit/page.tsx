...
import { Plus, Trash2, Sparkles, Save, Upload, FileJson, Info, Maximize2, Bold, Italic, Link2, ImageIcon, List, Eye, X, Loader2 } from "lucide-react";
import styles from "@/app/(main)/create/create.module.css";
import { useRouter, useParams } from "next/navigation";
import { AnimatePresence, motion } from "framer-motion";
import Cropper from "react-easy-crop";
import getCroppedImg from "@/lib/cropImage";
import { updateCharacterAction, getCharacterByIdAction } from "@/app/(main)/actions";
...
export default function EditCharacterPage() {
        ...
        async function loadChar() {
            ...
            if (res.success && res.data) {
                ...
                setFormData({
                    ...
                    characterChatName: char.characterChatName || "",
                    characterBio: char.characterBio || "",
                    creatorNotes: char.creatorNotes || "",
                    tags: char.tags || [],
                    publishSettings: char.publishSettings || "Public",
                    contentRating: char.contentRating || "Limited",
                    personality: char.personality || "",
                    scenario: char.scenario || "",
                    exampleDialogue: char.exampleDialogue || "",
                });
                if (char.firstMessages && char.firstMessages.length > 0) {
                    setFirstMessages(char.firstMessages);
                }
            } else {
    ...
    if (isLoadingData) return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', color: 'var(--text-secondary)' }}>Loading...</div>;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const result = await updateCharacterAction(params.id as string, submissionData);
        if (result.success) {
        ...
        <div className={styles.createContainer}>
            ...
            <div className={styles.createHeader}>
                <div className={styles.headerLeft}>
                    ...
                    <p className={styles.subtitle}><Link href={`/character/${params.id}`} style={{ color: "#3b82f6", textDecoration: "underline", display: "inline-flex", alignItems: "center", gap: "0.25rem", fontWeight: 600 }}>View Character</Link></p>
                </div>
                ...
                <div className={styles.formArea}>
                    <form onSubmit={handleSubmit} id="characterForm">
                        ...
                        <div className={styles.submitWrapper}>
                            <button type="submit" form="characterForm" className={styles.submitBtn} disabled={isSubmitting} style={{ backgroundColor: "#3b82f6" }}>
